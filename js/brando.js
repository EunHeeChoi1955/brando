...
;(function($, window, document, undefined){ //매개변수
    ...
    var brando = {
        ...
        section10Fn:   function(){
            var cnt=0;  //cnt는 여기section10에서만 쓴거
            ...
            var $window    = $(window);
                ...
                function resizeFn(){
                    ...
                    mainSlieFn();
                    //$('.slide-wrap').stop().animate({left:-( $slideConW*cnt)},0);
                    //화면 줄일때 슬라이드 애니매이션이 살짝 움직임 주고싶으면 줘도되고-> mainSlieFn()이거 없애고 줘야함  
                    //안줘도 되고-> 이때는 mainSlieFn()이거 넣기 
                }
                $window.resize(function(){
                    resizeFn();
                });        
        ...
        section11Fn:   function(){
            ...
            var $leftImgBox  = $('#section11 .left-box img');
            var $rightBox = $('#section11 .right-box');
            var $leftBoxW = $('#section11 .left-box').innerWidth(); //반응형 너비구하기
                $rightBox.css({height:$leftBoxW});   //반응형 높이 설정
                $leftBox.css({height:$leftBoxW});    //반응형 높이 설정
                $leftBox.css({height:$leftBoxW});    //반응형 높이 설정
                $leftImgBox.css({height:$leftBoxW}); //반응형 높이 설정
